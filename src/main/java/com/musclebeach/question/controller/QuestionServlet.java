...
@WebServlet({"/front-end/question/question.do", "/back-end/question/question.do"})
public class QuestionServlet extends HttpServlet {

	private final ApplicationContext ctx =  ApplicationContextUtil.getContext();

	private final QuestionService questionService = ctx.getBean(QuestionService.class);
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			QuestionVO questionVO = questionService.getOneQuestion(questionID);
			if (questionVO == null) {
				errorMsgs.add("查無資料");
			}
			// Send the use back to the form, if there were errors
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			questionVO = questionService.updateQuestion(questionID, questionTitle, questionContent);
		...
		if ("insert".equals(action)) { // 來自addquestion.jsp的請求
			...
			questionVO = questionService.addQuestion(questionTitle, questionContent);
		...
		if ("delete".equals(action)) { // 來自listAllQuestion.jsp
			...
			questionService.deletQuestion(questionID);
		...
		if ("listQuestionByQuestionContent".equals(action) || "listQuestionByQuestionContentB".equals(action)) {
			...
			if (QuestionContent == null || QuestionContent.toString().trim().length() == 0) {
			...
			Set<QuestionVO> set = questionService.getQuestionByQuestionContent(QuestionContent);
			if (set.size() == 0) {
		...
		if ("listQuestionByQuestionTitle".equals(action) || "listQuestionByQuestionTitleB".equals(action)) {
			...
			Set<QuestionVO> set = questionService.getQuestionByQuestionTitle(questionTitle);
			if (set.size() == 0) {
				RequestDispatcher successView = req.getRequestDispatcher("/back-end/question/nodata.jsp");
				successView.forward(req, res);
				return;
			...
			if ("listQuestionByQuestionTitle".equals(action)) {
				...
				RequestDispatcher successView = req
						.getRequestDispatcher("/front-end/question/listQuestionByQuestionTitle.jsp");
				successView.forward(req, res);
			} else {
