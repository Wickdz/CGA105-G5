...
@WebServlet({"/front-end/news/news.do", "/back-end/news/news.do"})
public class NewsServlet extends HttpServlet {

	private final ApplicationContext ctx = ApplicationContextUtil.getContext();
	private final NewsService newsService = ctx.getBean(NewsService.class);
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			newsVO = newsService.updateNews(newsID, empID, newsTitle, newsContent, newsTime);
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			newsVO = newsService.addNews(empID, newsTitle, newsContent);
		...
		if ("listNewsByEmpID".equals(action)) {
			...
			Set<NewsVO> set = newsService.getNewsByempID(empID);
			if(set.size()==0) {RequestDispatcher successView = req.getRequestDispatcher("/back-end/news/nodata.jsp");
			successView.forward(req, res);
			return;
			}
			/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
		...
		if ("listNewsByNewsTitle".equals(action)||"listNewsByNewsTitleB".equals(action)) {
			...
			Set<NewsVO> set = newsService.getNewsByNewsTitle(newsTitle);
			if(set.size()==0) { if("listNewsByNewsTitleB".equals(action)) {
				RequestDispatcher successView = req.getRequestDispatcher("/back-end/news/nodata.jsp");
			successView.forward(req, res);
			return;
			}else {RequestDispatcher successView = req.getRequestDispatcher("nodata.jsp");
			successView.forward(req, res);
			return;
			}
			}
