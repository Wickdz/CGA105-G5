...
<%@ page import="com.musclebeach.mem.model.*"%>
<%@ page import="com.musclebeach.room.model.*"%>
<%
...
<c:if test="${not empty errorMsgs}">
    ...
    <div class="container">
        ...
        <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/payRoomOrder" name="form1">
         	<div class="flexbox">
                <div class="inputBox">
                	<span>會員編號</span>
         		...
         		<div class="inputBox">
                	<span>交易金額（新台幣）</span>
                	<input type="text" maxlength="16" name="roomPrice" 
                 	value="${roomVO.roomPrice}"  readOnly/>
            	</div>
            </div>
            <div class="inputBox">
                <span>信用卡卡號</span>
				...
				oninvalid="setCustomValidity('請輸入卡號: 16碼數字');"
            ...
            <div class="flexbox" style="margin-bottom: 15px;">
                <div class="inputBox">
                    ...
                    pattern="[0-9]{4}"
					onchange="setCustomValidity('');"
					oninvalid="setCustomValidity('請輸入信用卡期限: 4碼數字');"
                ...
                <div class="inputBox">
                    ...
                    pattern="[0-9]{3}"
					onchange="setCustomValidity('');"
					oninvalid="setCustomValidity('請輸入驗證碼: 3碼數字');"
                    required/>
                </div>
            </div>
            <div>
                  <input class="form-check-input" type="checkbox" name="saveCard" id="flexCheckDefault"/>
                  <label class="form-check-label" for="flexCheckDefault" style="color: #999;">
                    儲存卡片資訊，以供未來使用
                  </label>
            </div>
            <input type="hidden" name="action" value="insert_room_order">
            <input type="hidden" name="roomID" value="<%=roomVO.getRoomID()%>">
            <input type="hidden" name="borrowDate" value="<%=request.getAttribute("borrowDate")%>">
            <input type="hidden" name="borrowTime" value="<%=request.getAttribute("borrowTime")%>">
