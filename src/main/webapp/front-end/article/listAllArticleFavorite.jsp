...
while(iterator.hasNext()){
	ArticleFavoriteVO articleFavoriteVO = iterator.next();	
		Iterator<ArticleVO> iterator2 = articleFavoriteVO.getArticleVO().iterator();
		while(iterator2.hasNext()){
			ArticleVO articleVO = iterator2.next();
...
<body class="bg-dark" style="--bs-bg-opacity: .95; overflow-x: hidden;">
 ...
 <c:if test="${ memVO.memID!=null}">
  ...
  <main style="top: 73.333px; position: absolute;">
	...
	<h1 style="display: flex; justify-content:center ;">暫無收藏文章</h1>
	</c:if>
 
 <c:forEach var="articleFavoriteVO" items="${list}">
    <c:forEach var="articleVO" items="${articleFavoriteVO.articleVO}">
